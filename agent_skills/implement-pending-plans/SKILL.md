...
description: Implement pending plan stages under docs/plans/; walk one *-pending plan in order; Status in_progress → done/blocked; no auto git.
...
Companion to `create-multi-stage-plan`. Plans are directories under `docs/plans/` whose names end in `-pending`. Stage files live inside the plan directory; do **not** assume a fixed stage filename template — find markdown stages under the chosen plan (including nested paths) and order them sensibly.
...
- When every stage file is `done`, rename by replacing the trailing `-pending` with `-done`.
- If a `*-pending` plan already has all stages `done` on discovery: rename to `*-done` and report nothing else to do.
...
2. Under each plan directory, find stage markdown files and read each file’s Status.
...
On the chosen plan, order stages sensibly (e.g. numbers in names or headings, dependencies in content). No required filename grammar.
...
3. If you cannot finish (underspecified, missing dependency, hard failure): set Status to `blocked`, explain to the user, **stop the plan** (do not continue later stages). Leave the plan directory as `*-pending`.
4. Otherwise set Status to `done` and continue.

After the walk (or after a single named stage): if every stage on the plan is `done`, rename `*-pending` → `*-done`.
