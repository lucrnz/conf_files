...
This skill accepts one argument:
...
- `thermo-nuclear review` → `scope=changes`
...
Use this when the user made edits on the current branch without opening a PR and wants the working tree reviewed.

Before reviewing:

1. Run `git status --short` to see modified, staged, deleted, and untracked files.
2. Run `git diff` for unstaged changes and `git diff --cached` for staged changes.
3. For new untracked files that are part of the work, read those files directly.
4. Treat the union of staged + unstaged + relevant untracked files as the review surface.
5. Do **not** require a feature branch or PR. Review whatever is currently in the workspace.
6. When a touched file is large, read enough surrounding context to judge whether the change worsens decomposition, but keep findings anchored to what this change set introduced or failed to fix.

Focus questions for this scope:

- Did this change set make the touched area simpler or messier?
...
Use this for periodic full-application health audits across the repo.

Before reviewing:

1. Identify the major application surfaces (TypeScript backend/frontend, Python extension supervisor, startup/bootstrap, adapters).
2. Prioritize the largest and most central modules, recent architectural moves, and cross-cutting boundaries.
3. Measure file size, ownership boundaries, and repeated orchestration patterns repo-wide.
4. Do not limit findings to a single diff. Look for structural health problems that compound over time.

Focus questions for this scope:

- Which modules are god files or on a direct path to becoming god files?
- Where did recent refactors move complexity instead of deleting it?
- Which boundaries leak feature logic into shared paths?
- What are the highest-leverage decomposition or code-judo opportunities anywhere in the app?

## Core Prompt

Start from this baseline, then apply the scope workflow above:

> Perform a deep code quality audit of the selected review scope.
...
For every meaningful change or module in scope, ask:
...
- A file crossing 1000 lines because of the change set, especially if the new code could be split out.
...
- the change set preserves a lot of incidental complexity when there is a plausible code-judo move that would delete it
- the change set pushes a file from below 1000 lines to above 1000 lines
- the change set adds ad-hoc branching that makes an existing flow more tangled
- the change set solves a local problem by scattering feature checks across shared code
- the change set adds an unnecessary abstraction, wrapper, or cast-heavy contract that makes the design more indirect
- the change set duplicates an existing helper or puts logic in the wrong layer when there is a clear canonical home
