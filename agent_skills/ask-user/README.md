...
The agent prints an `ask-user: ` HTTPS URL in chat. Open that URL. Do not open `127.0.0.1` or the ephemeral loopback port. The printed URL is normally a `https://v.gd/…` short link to the tunneled wizard; v.gd may show a preview / continue page first. If v.gd cannot create the link, the line is a `https://tinyurl.com/…` URL. If both fail, the line is a long `*.trycloudflare.com` URL — open that instead.
